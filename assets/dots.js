...
function getRandomDotsFromColor(color, count) {
    ...
    const shuffled = [...dotsOfColor].sort(() => 0.5 - Math.random());
    return shuffled.slice(0, Math.min(count, dotsOfColor.length));
}

function getRandomDots() {
    return [
        ...getRandomDotsFromColor('green', Math.floor(Math.random() * 3) + 2),    // 2-4 green
        ...getRandomDotsFromColor('red', Math.floor(Math.random() * 4) + 2),      // 2-5 red
        ...getRandomDotsFromColor('yellow', Math.floor(Math.random() * 3) + 2),   // 2-4 yellow
        ...getRandomDotsFromColor('blue', Math.floor(Math.random() * 3) + 2),     // 2-4 blue
        ...getRandomDotsFromColor('purple', Math.floor(Math.random() * 3) + 2),   // 2-4 purple
        ...getRandomDotsFromColor('pink', Math.floor(Math.random() * 3) + 2),     // 2-4 pink
        ...getRandomDotsFromColor('lightGreen', Math.floor(Math.random() * 3) + 2)// 2-4 lightGreen
    ];
}
...
function drawDots() {
    ...
    ctx.globalAlpha = 0.9;
    ctx.filter = `blur(${8}px)`;

    const isMobile = window.innerWidth <= 768;
    
    let scale;
    if (isMobile) {
        scale = window.innerHeight / SVG_HEIGHT;
    } else {
        scale = (window.innerWidth / SVG_WIDTH) * 0.8;
    }
    
    const offsetX = (canvas.width - SVG_WIDTH * scale) / 2;
    const offsetY = isMobile ? 0 : (canvas.height - SVG_HEIGHT * scale) / 2;

    const dotsToShow = getRandomDots();

    dotsToShow.forEach(dot => {
        const jitterX = (Math.random() - 0.5) * scale * 2;
        const jitterY = (Math.random() - 0.5) * scale * 2;

        ctx.beginPath();
        ctx.arc(
            dot.x * scale + offsetX + jitterX,
            dot.y * scale + offsetY + jitterY,
            DOT_RADIUS * scale,
            0,
            2 * Math.PI
        );
